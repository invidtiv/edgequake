...
> **Product: v0.21.3** · Contract: OpenAPI · Spec ops: [Ingestion cancel & fairness](../ingestion-cancel-and-fairness.md)
...
```bash
gh release view v0.21.3
docker buildx imagetools inspect ghcr.io/raphaelmansuy/edgequake:0.21.3
docker buildx imagetools inspect ghcr.io/raphaelmansuy/edgequake-frontend:0.21.3
docker buildx imagetools inspect ghcr.io/raphaelmansuy/edgequake-postgres:0.21.3
docker buildx imagetools inspect ghcr.io/raphaelmansuy/edgequake-postgres:0.21.3-pg16
docker buildx imagetools inspect ghcr.io/raphaelmansuy/edgequake-postgres:0.21.3-pg17
```
...
git tag v0.21.3 && git push origin v0.21.3
```

Both `linux/amd64` (ubuntu-latest runner) and `linux/arm64` (native ARM64 runner — no QEMU) are built in parallel and merged into a single multi-arch manifest. The same image tag (`ghcr.io/raphaelmansuy/edgequake:0.21.3`) works on x86 servers, Apple Silicon Macs, and AWS Graviton instances.
